...
<%
	...
	String sql = "select id,username,userpassword,role,mobile from userinfo where id="
			+ id;
...
%>
	...
	<div id="content">
		...
		if (rs.next()) {
	%>
	<form action="doedit.jsp"  style="margin-left:50px;">
		<input type="hidden" name="id" value="<%=id%>"/>
		用户名：<%=rs.getString("username")%>
		<br /> 
		用户密码<input type="password" name="userpassword" value="<%=rs.getString("userpassword")%>" /> <br /> 
		用户手机<input type="text" name="mobile" value="<%=rs.getString("mobile")%>" /> <br />
		<input type="submit" value="更新用户信息"/>
	</form>
	<%
		} else {
			request.setAttribute("info", "你查询的用户并不存在，请核对后重试！");
